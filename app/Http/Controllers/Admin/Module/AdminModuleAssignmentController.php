...
class AdminModuleAssignmentController extends Controller
{
    ...
    {
        try {
            $assignments = $this->assignment->assignmentSubmissionsByModuleId($request->module_id)
                ->latest()->paginate(20);
            return response()->json([
                'success' => true,
    ...
    public function searchSubmissions(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $data = $this->assignment->searchSubmittedAssignments($request);
