...
class UserService
{
    ...
    {
        return $this->braceUser()->with('module_assessments', 'business', 'course_assessments', 'completed_module_assessments')
            ->where('selected', 1);
    }

//    public function selectedUsersNotStartedProgram(): \Illuminate\Database\Eloquent\Builder
//    {
//        return $this->braceUser()->with('business')
//            ->doesntHave('course_assessments')
//            ->where('selected', 1);
//    }
    ...
    public function updateAnyField($request, $userId): array
    {
        ...
        $updatedInput = [];
        // Add password field to array, for some reason password field disappears when user object is converted to array
