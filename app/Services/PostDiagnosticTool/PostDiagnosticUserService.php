 ...
 */
class PostDiagnosticUserService extends BaseService
{
    public function postDiagnosticUser(): PostDiagnosticUser
    {
    ...
    {
        return $this->postDiagnosticUser()->with('user', 'answers', 'last_question');
    }

    public function hiddenQuestions(): PostDiagnosticHiddenQuestion
    {
        return new PostDiagnosticHiddenQuestion();
    }

    public function postDiagnosticAnswer(): PostDiagnosticAnswer
    {
    ...
    {
        $user = $this->userWithRelations()->findOrFail($id);
        // Delete hidden questions
        $this->hiddenQuestions()->where('user_id', $user->user_id)->delete();
        // Delete user answers
        $answers = $this->postDiagnosticAnswer()->where('user_id', $user->user_id)->get();
        if($answers->count() > 0){
